...
const Home = function () {
  ...
  const [monthlyRevenue, setMonthlyRevenue] = useState<any>(0);

  const fetchTotalRevenue = async () => {
    
    let results = await apiGetTotalRevenue();
    
    setTotalRevenue(results.totalRevenue);
    //alert(JSON.stringify(results.totalRevenue) );
  ...
  const getMonthlyRevenue = async () => {
    ...
    const currentDate = new Date(); // Lấy ngày hiện tại của hệ thống
    const endDate = currentDate.toISOString().slice(0, 10); // Chuyển đổi ngày thành chuỗi ISO (YYYY-MM-DD)

    // Lấy ngày đầu tiên của tháng từ ngày hiện tại
    ...
    let results = await apiGetTotalRevenueByDateRange(startDate, endDate);

    setMonthlyRevenue(results.totalRevenue);
